        ...
        .then((res) => {
            if (!res.error) {
                if (res.port) {
                    ...
                    var win = window.open(`${window.location.origin}:${res.port}/${e.srcElement.children.filename.value}`, 'popup', `left=${window.screen.width},top=${window.screen.height},width=600,height=700`);
                    var done = false;
                    win.focus();
